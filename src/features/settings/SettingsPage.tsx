        ...
        <main className="flex h-dvh min-h-0 flex-col bg-canvas text-zinc-100" aria-labelledby="settings-title">
            <header className="flex h-[68px] shrink-0 items-center gap-3 border-b border-line px-5">
                ...
                <div>
                    ...
                    <h1 id="settings-title" className="mt-1 text-sm font-semibold text-zinc-100">
                        Settings
                    </h1>
                </div>
            </header>
            ...
            <div className="min-h-0 flex-1 space-y-6 overflow-y-auto px-5 py-5">
                {props.actionError && (
                    <div
                        className="flex items-start gap-3 border-l-2 border-rose-400 bg-rose-400/5 px-3 py-2.5 text-[10px] leading-4 text-rose-200"
                        role="alert"
                    >
                        <p className="min-w-0 flex-1">
                            <span className="font-semibold capitalize">
                                {props.actionError.action.replace(/-/g, " ")}:
                            </span>{" "}
                        ...
                        <button
                            ...
                            className="text-rose-300 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-hanami"
                        >
                            Dismiss
                        </button>
                    </div>
                ...
                <AdvancedSettings
                    snapshot={props.snapshot}
                    pending={props.pending}
                    onGrantMemoryAccess={props.onGrantMemoryAccess}
                    focusRequested={props.focusAdvanced}
                />
                <AboutSettings
                    snapshot={props.snapshot}
                    pending={props.pending}
